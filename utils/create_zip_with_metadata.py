...
def create_zip_with_metadata(project_path: Path, output_zip_filename: Path) -> None:
    ...
    logger.info(f"Source directory: {project_path.absolute()}")
    logger.info(f"Output ZIP file: {Path(output_zip_filename).absolute()}")

    files_to_zip: List[Path] = []
    # Walk through the source directory to find all files
    for root, _, files in os.walk(project_path.absolute()):
        relative_path = Path(root).relative_to(project_path)
        # ignore env
    ...
    if not files_to_zip:
        raise EmptyProjectDirError(
            f"Warning: No files found in '{project_path.absolute()}'."
        )
    ...
    try:
        with zipfile.ZipFile(output_zip_filename, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for file_path in files_to_zip:
                ...
                arcname = project_dir / file_path.relative_to(project_path)
