...
def get_python_executable(python_version: str) -> Path:
    ...
    python_version = python_version_mapping.get(python_version, python_version)
...
def uninstall_local_packages_from_requirements(
    project_dir: Path, python_executable: Path, requirements_file: Path
) -> None:
    """
    Uninstalls packages from a requirements file in the local Python environment.
    """

    if not requirements_file.exists():
        logger.error(
            f"Requirements file '{requirements_file}' does not exist. Cannot uninstall packages."
        )
        return

    logger.info(f"Uninstalling packages from {requirements_file}...")
    ...
    if not requirements.intersection(installed_requiments_set):
        logger.info(
            f"No packages from {requirements_file} are installed in {project_dir}."
        )
        return
    ...
    try:
        ...
        logger.error(
            f"Failed to uninstall packages from {requirements_file}. Error: {e.stderr}"
        )
        raise UnableToInstallPackagesError(
            f"Failed to uninstall packages from {requirements_file}. Error: {e.stderr}"
        )
...
def uninstall_local_python_packages(
    project_dir: Path, python_executable: Path, packages: List[str]
) -> None:
    """
    Uninstalls specified packages in the local Python environment.
    """
    if not packages:
    ...
    try:
        uv_command = get_uv_command()
        subprocess.run(
            [uv_command, "pip", "uninstall", "--python", str(python_executable), "-y"]
            + packages,
            check=True,
            cwd=project_dir,
            capture_output=True,
        ...
        logger.error(f"Failed to uninstall packages {packages}. Error: {e.stderr}")
        raise UnableToInstallPackagesError(
            f"Failed to uninstall packages {packages}. Error: {e.stderr}"
        )
