...
detect_os() {
    ...
    case "$OS" in
        ubuntu)
            ...
            DATALLOG_INSTALL_UV="install_uv_linux"
        ...
        macos)
            ...
            DATALLOG_INSTALL_UV="install_uv_macos"
...
install_deps_debian() {
    ...
    package_list="ca-certificates curl x11-xserver-utils gnupg lsb-release build-essential zlib1g-dev libncurses-dev libgdbm-dev libnss3-dev libssl-dev libsqlite3-dev libreadline-dev libffi-dev libbz2-dev pkg-config liblzma-dev uuid-dev"
    package_to_install=""
...
install_uv_macos() {
    ...
    if ! command -v uv &>/dev/null; then
        export NONINTERACTIVE=1
        brew install uv || {
            echo "Failed to install uv using Homebrew. Please check your Homebrew installation."
            exit 1
        }
    fi
}
...
install_uv_linux() {
    ...
    if [ -z "$CURL" ]; then
        echo "The installer needs either curl or wget to download files." >&2
